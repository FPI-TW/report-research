...
- The `claude` CLI backend was removed in PR-M (together with the web unit's PATH drop-in and probe exit code 5); DeepSeek is the only LLM backend. `/healthz` probes only the DB. R2: `/healthz` never probes it; the probe reads loopback-only `/healthz/storage` and exits 6 when the bucket or credentials are unusable. The DeepSeek account is read from loopback-only `/healthz/llm`: balance below the CNY floor exits 7 (WARNING), exhausted/401/unreachable/indeterminate exits 8 (CRITICAL); the probe checks both, reports every reason on one line, and exits with the first of 8 → 6 → 7 (exit code 5 is retired and not reused).
