...
| 新增 LLM 呼叫點（`stream_completion`） | 帶 `max_tokens`（依第二版計畫 §8）與 `task`（`tests/test_llm.py` 靜態釘住）；同步更新所有假物件簽章；model 只用 DeepSeek 白名單（`llm_models.HTTP_MODELS`），其他名稱一律 config 錯誤 |
...
- 網搜每題由使用者決定：`web_on` ＝ 請求的 `web` AND `ASK_ENABLE_WEB`，下游只讀 `web_on`。**網搜目前沒有後端**（Claude CLI 的 WebSearch 隨 PR-M 移除，DeepSeek 版延後到 P9）：`ASK_ENABLE_WEB` 預設 0、`ASK_WEB_MODEL` 預設空字串，`stream_completion(allow_web=True)` 不論 model 一律拋 `kind=config`——時效題退回 M4 婉拒、主答回「設定有誤」；前端 `frontend/src/lib/useWebSearch.ts` 的 `WEB_SEARCH_PAUSED` 隱藏開關、請求一律送 `web=false`。P9 完成後三處一起恢復。系統提示與工具授權要一起切（`ask_system_prompt(web)`），逾時只在開網搜時放寬（`ASK_WEB_TIMEOUT`），`qa_log.filters.web` 含 False 也要寫，免責句由 Python 追加（`WEB_ANSWER_DISCLAIMER`），網搜來源不進 evidence ledger。
...
- `LLM_PROVIDER` 只剩一個合法值 `deepseek`（未設、空值都是）。claude CLI 已於 2026-09-23 放棄、PR-M 移除 backend：`claude_cli`／`claude_only` 退役（`llm_models.RETIRED_PROVIDERS`），web 記 ERROR 並照常當 deepseek，批次預檢 rc=2 並印原始值（拼錯同樣處理）——想讓 LLM 停下來卻變成照常計費是最壞的結果，所以批次不猜；沒有可回退的後端，要停批次就停 `report-mark-sync.timer`。`/etc/default/report-mark-llm` 缺檔時批次因缺金鑰 rc=2。預設表只有 DeepSeek（網搜那列空字串）；兩個 judge 是 `deepseek-flash`（新量尺，讀分數三處只計現行 judge，歷史 haiku 列歸「其他 judge」；judge 走 `llm_http.complete_json`，每階段最多 3 個請求，judge model 不在白名單時不送出）。`tests/conftest.py` 強制 `LLM_PROVIDER=deepseek`、金鑰空、端點不可達：漏了假物件的測試在送出前就以 auth 失敗，要驗 LLM 行為的測試自己裝 `httpx.MockTransport` 或 patch 呼叫層。
- `app/services/llm.py` 的 `stream_completion` 只走 DeepSeek（`llm_http`）：model 不在 `llm_models.HTTP_MODELS` 或 `allow_web=True` 時送出前就拋 `LLMUnavailableError(kind="config")`；呼叫點一律帶 `max_tokens` 與 `task`。錯誤處置：只有 overloaded／network 且尚未吐字才重試；未吐字失敗拋 `LLMUnavailableError(kind=…)`，`/api/ask` 依 kind 給訊息（內容審查請使用者換個問法、quota／auth 說帳號異常、config 說設定有誤，都不承諾已通知）；首字前 httpx read 逾時歸 timeout 不重試；已吐字後被內容審查拋 `partial=True`、`length`／read 逾時／總時限（`LLM_HTTP_TOTAL_TIMEOUT`）正常結束帶 `meta["truncated_reason"]`，總覽／時效／主答保留已吐的字、由 Python 附註中斷原因並寫 `filters.llm_truncated`；內容審查一律標記、跳過、交人工。`timeout` 是首字期限（經 `_with_heartbeat` 驅動也準時）。`LLMUnavailableError.reason` 是 CLI 時代的相容欄位（faithfulness、eval 仍讀），新程式讀 `kind`。批次的呼叫層是 `scripts/_claude_cli.py` 的 `run_claude`（名稱是歷史遺留，同樣只收白名單，其他名稱整批 rc=2）。
...
- 簡報（`app/services/brief.py`）：窗期用 `created_at` 不是 `report_date`；變動要同時「這輪才擷取」且「報告夠新」；來源清單由 Python 記錄不從 markdown 反推。鎖只包那一次 LLM 呼叫。
...
- DB 一律 `from app.services.db import SessionFactory`，不複製預設連線字串（鍵是 `REPORT_MARK_DB_URL`）。長查詢用 `db.relax_statement_timeout()`（`SET LOCAL`）。`DB_IDLE_TX_TIMEOUT_MS` 預設 0 是刻意的：sync 在交易內呼叫 LLM 標註與嵌入。
