...
**研報生成跑在背景任務，不綁 SSE 連線**（`web/report_runs.py`）：`POST /api/report` 只是訂閱端，斷線／重整不再中止生成；`GET /api/report-runs?conversation_id=` 探詢進行中的 run、`/api/report-runs/{id}/stream` 重連（重播＋直播）、`/api/report-runs/{id}/cancel` 主動中止。登錄表是**行程內**狀態，重啟即全滅——`report_writer.open_run` 因此把「久無心跳（`REPORT_RUN_STALE_SECONDS`）的 in-flight run」也視為可重試，否則一次 deploy 就讓該冪等鍵永遠卡在「正在處理」。前端進度靠 `outline`（分母）＋ `section_draft`/`section_skipped`（分子）兩類事件（`lib/reportProgress.ts` 是純函式，元件只負責每秒重算）。
...
- **前端要看到的事件，`parseReportEvent`／`parseAskEvent` 必須有對應分支。** 未知 event 一律回 `null` 被靜默丟棄——後端送了 `section_draft` 好幾個里程碑，前端從來沒解析過，症狀只是「進度條停在 50% 不動」，沒有任何錯誤。加後端事件時同步加 parser，並用測試釘住。
