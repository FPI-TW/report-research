...
# 陷阱：normalize 一律不要跑（使用者要求時請先說明再婉拒）
make normalize                       # 自稱冪等，實際會抹掉幾乎所有 chunk 的段落結構，見下方 gotcha
```
...
- **`make normalize` 是陷阱，看到也不要跑。** target 說明與 `scripts/normalize_chunks.py` 的 docstring 都寫「冪等、重跑 0 筆更新」，但腳本用的是 `clean_text`，而 chunk 是 `chunk_text(clean_extracted(...))` 產生的——`clean_text` 會把換行折成空格，而 chunk 內的段落正是用單一換行接起來的，跑一次就把現有語料幾乎每一列的段落結構整個抹掉，同時讓表與 HNSW 索引雙倍膨脹，而 `content_norm` 完全不變（該表達式本來就移除所有空白）＝零收益。逐步推導與實測見 `docs/ARCHITECTURE_REVIEW_2026-07.md` 的 P0 第 1 項。
