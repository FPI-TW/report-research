...
- **logging 從未初始化，`logger.info` 等於寫進黑洞。** 全 repo 沒有任何 `basicConfig`／`dictConfig`，而 uvicorn 只設定自己的 logger、不動 root——`app.services.*` 因此走 `logging.lastResort`（level=WARNING、只印裸訊息）。`answer.py` 的 `qa_timing` 與 `qa_agentic` 兩行分段耗時遙測**一行都沒進 journald**。要靠日誌排查前，先在 `web/server.py` 補 `dictConfig`，否則你加的 `logger.info` 不會出現。
