...
**檢索**：搜尋框在內容區上方，結果預設以**列表**（依市場／報告類型／日期(月)分組，右上可切「分組依據」）呈現，也可切**表格**（右上角圖示切換）。每筆顯示市場標籤、商品類型、標的、券商來源、報告日期、命中片段數、相關度 %，以及**2-3 句中文摘要**（列表完整顯示；表格於名稱 hover 顯示）讓你不必開全文就能掌握大意；點任一筆即**內嵌原始 PDF**（彈窗頂部亦顯示摘要）。摘要由 `make summaries` 離線生成。
