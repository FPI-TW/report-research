...
│   review_extraction_golden.py  golden set 覆核頁：頁面渲染＋片段框選標序號 → data/extraction/review/index.html
...
| GET | `/api/progress` | 監控快照（DB 筆數、摘要／重點摘錄／訊號覆蓋、背景程序、忠實度查核，外加 `sync`＝每 3 小時排程同步的最新狀態、`unit_failures`＝`OnFailure` 告警的近期計數、`db.sources`＝券商分佈、`extraction`＝抽取品質與回填進度（E1，缺表時 `null`）、`pipelines.backfill`） | |
...
- **CI**（`.github/workflows/ci.yml`）有**四個 job**：前端測試（ESLint + tsc + **vite build** + vitest）、後端測試（**ruff** + pytest）、**schema 契約**（`pgvector/pgvector:pg16` service container，套 `db/schema.sql` 兩次驗冪等 ＋ `content_norm` 等價性 ＋ CHECK 約束清單對帳）、**secret 掃描**（gitleaks，全歷史 ＋ 工作樹兩步）。前端 job 把 `frontend/dist` 當 artifact 傳給後端 job，`tests/test_spa_serving.py` 因此對**真 build 產物**驗證（缺 dist 是**紅**不是 skip；本機要跳過設 `SKIP_SPA_TESTS=1`）。**四個 job 皆為必要檢查**（前三個 2026-07-29 起、secret 掃描 2026-07-31 起；required check 名稱是 job 的中文 `name`，改名等於讓分支保護指向一個永不回報的 check，**改 `name` 就要同步改 GitHub 分支保護設定**）。main 有分支保護（strict ＋ enforce_admins）。**本機只跑 pytest 會在前端 job 上翻車。**
...
| NAS 增量同步 | systemd `report-mark-sync.timer`（每 3 小時）→ `scripts/sync_new_reports.sh`（drvfs 唯讀掛載 → rsync delta → 增量 extract/tag/ingest → 補摘要 → 補顯示標題 → 補重點摘錄 → 觀點訊號擷取 → 每日簡報 → 補標題歷史積壓）；手動測試 `make sync-once`。**中間三段補缺值的批次吃本輪 `--hashes-file`**（不可改成 `--since-days`，它濾的是 `report_date` 而非入庫時間）；**最後三段刻意不同**——訊號與標題積壓排的是跨全語料的積壓、每日簡報則一天只跑一次，故不綁 `--hashes-file`、也不在「有新研報才跑」的判斷內，改以 `--limit`（`SYNC_SIGNAL_LIMIT` 預設 100、`SYNC_TITLE_BACKLOG_LIMIT` 預設 60）限量，那兩個上限是安全機制不是效能旋鈕（值由實測吞吐與 3 小時視窗回推）。**這條鏈才是生產實際的入庫路徑**，全量三支腳本只在初次建庫或補跑歷史時用。見 [docs/nas_scheduled_sync_deployment.md](docs/nas_scheduled_sync_deployment.md) |
