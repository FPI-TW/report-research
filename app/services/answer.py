...
#
# **「查不到就說找不到」必須住在分支裡，不能留在共同規則。** M11 上線時共同規則第 1
# 條寫著「查不到時，明說『找不到相關資料』」，而 WEB_POLICY 只說「片段不足才搜尋，
# 不要無謂搜尋」——兩條在「片段查無此標的」這個最需要網搜的情境下直接衝突，模型會
# 選比較明確的那條：直接回「找不到相關資料」，一次搜尋都不發。2026-08-21 以「分析
# 兆勁」實測（語料裡確實有一篇兆勁研報，只是檢索沒撈到），web=true 下 `stages` 完全
# 沒有 `searching_web`：**5 次全滅**。把第 1 條的終局指令移進 NO_WEB_POLICY、並把
# WEB_POLICY 第 6 條改成「片段不足就必須先搜」之後，同一題 7 次有 6 次搜網並帶回
# [EXT_SOURCES]。**6/7 不是 7/7**——這仍是機率性的，prompt 寫了不等於保證（同 zh_hant
# 與 faithfulness 的教訓）；真正的決定性修法是讓檢索先撈得到，見 retrieval.extract_terms
# 對純 CJK 不斷詞那條缺口。**症狀是靜默的**：使用者只看到「找不到相關資料」，與網搜
# 壞掉完全同形，所以下面兩段的措辭是契約，改動前先看 test_ask_web_search.py。
SYSTEM_PROMPT_BASE = (
...
NO_WEB_POLICY = (
    "\n6. 本輪沒有網路存取能力：片段中查不到時，直接明說「找不到相關資料」；"
    "不得聲稱查過網路、不得標註『（網路）』、"
...
WEB_POLICY = (
    "\n6. 本輪使用者已開啟網路搜尋。先用研報片段作答；"
    "**只要片段不足以回答問題——查無該標的、資訊不完整、或可能已過時——就必須先執行網路搜尋**，"
    "不可略過搜尋直接回答「找不到相關資料」，搜尋後仍查不到才說查不到。"
    "片段已足以完整回答時則不必搜尋。\n"
    ...
    "9. 先完成必要的搜尋再開始作答，直接給結果；不要在答案中描述自己的檢索或搜尋流程"
    "（例如「依規則需先進行網路搜尋」），也不要先寫一版「找不到」再改寫。"
)
