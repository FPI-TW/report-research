...
# ── 數值主張偵測（確定性，零 LLM）──
# 金融數值：百分比、貨幣/單位金額、EPS/目標價/評等數字。純序號（如「第 3 點」）盡量不誤判：
# 只要含「數字＋金融語境或百分比或貨幣單位」即視為數值主張。
_NUMERIC_RE = re.compile(
    r"\d+(?:\.\d+)?\s*%"                       # 30% / 12.5 %
    r"|[\$￥€]\s*\d"                            # $123
    r"|\d+(?:\.\d+)?\s*(?:元|億|萬|兆|美元|台幣|新台幣|人民幣|港元|倍|bps|BP)"  # 金額/倍數
    r"|(?:EPS|每股|營收|毛利率?|淨利|目標價|本益比|殖利率|年增|季增|年減|季減|市佔率?)"
    r"[^。\n]{0,12}?\d"                          # 金融名詞近旁的數字
)
