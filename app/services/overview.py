...
_NAME_STOPWORDS: tuple[str, ...] = (
    ...
    + ("研報", "報告", "研究", "給我", "我", "想", "請", "幫我", "關於",
       ...
       "去年", "本週", "這週", "一覽")
    # 「券商動作」動詞：不剝掉的話會黏進公司名。實測「有哪些券商看台積電」→
    # stock_name='看台積電' → company_name ILIKE '%看台積電%' → 0 列 → 回「找不到」，
    # 而正解是列出看台積電的券商。「看/提到/寫過/談到」四種問法皆中招。
    # 長詞排前無妨——_extract_stock_name 以 len 由長到短剝除。
    #
    # 安全性對**實際被過濾的欄位** research_report.company_name 驗證過（454 個相異值）：
    # 下列 19 詞零誤傷。**「推薦」刻意不列入**——它會咬到真實值「年第二季推薦個股」
    # 「年第四季推薦個股」。（別拿 stock_targets 驗證：那欄存的是 4 位股票代碼而非
    # 中文名，用中文詞去比對數字代碼恆為零匹配，什麼都證明不了。）
    + ("看好", "看壞", "看多", "看空", "提到", "提及", "談到", "報導",
       "追蹤", "覆蓋", "關注", "分析", "評論", "寫過",
       "看", "提", "談", "寫", "評")
)
