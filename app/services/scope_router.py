...
fail-open：判定失敗/逾時/空回應一律視為 CORPUS_QA。寧可偶爾多答一題禮貌的「未提及」，
也不要因判斷器抖動而誤擋真實的研報提問。時效優先於 fail-open：safety_precheck 對
報價/個人化詞彙的命中總是優先，即使後續逾時也不降級。
"""
...
# 移除「盤中」「現價」「成交價」裸詞（M4 最終審查實測誤攔「大盤中長期趨勢」、「體現價值投資」、
# 「歷史成交價量分析」）；報價題改交 LLM 分類器判定，前檢維持近零誤攔原則。
