...
簡體）。prompt 是機率性保證，本模組是確定性的後處理，比照本 repo 對研報版面的
一貫作法（用 Python 收尾，不去改四份平行的 prompt 副本）。
...
2. **門檻是「至少 2 個字」且「密度至少 5%」，兩條缺一不可。**
   ...
   *密度*那條擋長文件：字數門檻是拿標題／摘錄（25-35 個漢字）校準的，**放到
   幾千字的研報 markdown 上就形同虛設**——一份純繁體研報只要提到「恒耀」兩次
...
- `report_section.draft_markdown`：逐節狀態機留的是「模型當時產出什麼」，轉了
  就不再是那個東西。使用者面的真相是 `report_doc.markdown`（組裝時已轉）。
"""
