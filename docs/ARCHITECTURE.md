...
| `llm.py` | `stream_completion` 只走 DeepSeek（`llm_http.astream_chat`）：model 不在白名單（`llm_models.is_http_model`；含 `claude-*`、空字串）或 `allow_web=True`（網搜沒有後端，DeepSeek 網搜延後到 P9）時送出前就拋 `kind=config`、不 yield。只有 overloaded／network 且尚未吐字才重試（Retry-After 優先、上限 10 秒，否則 1.5／3 秒）；非預期例外一律包成 `LLMUnavailableError`（`kind`、`partial`；`reason` 是 CLI 時代的相容欄位，由 kind 換算），`CancelledError` 原樣上拋並經 `aclosing` 關閉回應；已吐字後內容審查拋 `partial=True`，`length`／read 逾時／斷線／總時限（`LLM_HTTP_TOTAL_TIMEOUT`，預設 600 秒，每個 await 各包 `timeout_at`）正常結束並寫 `meta["truncated_reason"]`；首字前伺服器 60 秒沉默（httpx read 逾時）歸 `timeout`、不重試。`timeout` 是**首字期限**，經 `_with_heartbeat` 驅動也準時（每個 await 各包 `timeout_at`、不跨 yield）。PR-M 前另有 claude CLI 路徑（子行程、stream-json、`--tools`／`--strict-mcp-config` 旗標、529 重試），已整段移除；`SEARCH_EVENT` 保留給 P9 網搜 |
...
五類與 `decided_by`（`precheck`／`overview`／`llm`／`fail_open`／`unknown`）全寫進 `qa_log.filters`；`filters.web` 含 False 也要寫。有呼叫模型的輪次寫 `filters.llm_model`（實際送出的模型名；總覽退回模板時不寫）；已吐字後被截斷（內容審查、`length`、read 逾時、斷線、HTTP 總時限；`meta` 只帶 `truncated=True` 而沒有原因時記 `timeout`）時保留已送出的文字、由 Python 追加一行中斷註記（`answer.truncation_note`，時效網搜排在免責句之前），並寫 `filters.llm_truncated`；未吐字的失敗照舊落 `answer=NULL`＋`filters.llm_error`（`LLMUnavailableError.kind`）。網搜的逾時只在 `web_on` 時放寬（`ASK_WEB_TIMEOUT`；經 `_with_heartbeat` 驅動時它實際是「第一個輸出（文字或網搜標記）」的期限，之後不設牆鐘上限），免責句由 Python 追加（`WEB_ANSWER_DISCLAIMER`），網搜來源不進 evidence ledger，`drop_abandoned_draft` 只在 `web_on` 時套用。
...
忠實度抽查在 `done` 後以 `answer._spawn_background` 跑：四閘 `ASK_FAITHFULNESS_ENABLED`、有 `qa_id`、`is_numeric_claim(body)`、抽樣率；in-flight 超過 `ASK_FAITHFULNESS_MAX_INFLIGHT` 就跳過不排隊。結果落 `qa_log.evaluation`，judge 異常標 `degraded` 不加分數（原因記 `degraded_reason`）。judge 走 `llm_http.complete_json`（非串流、`json_object`、`temperature=0`、thinking 關、`user_id` 生產 `web-faithfulness`／離線 `eval-judge`，各階段 `max_tokens` 拆解 8192、grounding 2048、CP 2048、反推問題 1024）；judge model 不在白名單時不送出（生產記 `account`、離線拋 `JudgeAccountError`）。重試只有一層預算：每個階段（一次 `call_validated`）最多 3 個請求（`judge_schema.HTTP_STAGE_MAX_REQUESTS`，adapter 的截斷 2 倍重試／空回應／暫時性重試與 schema 重試共用、不相乘，`judge_json` 自 PR-M 起沒有自己的重試），所以生產一次抽查最壞 6 個請求。審查記 `degraded_reason=content_risk`、401／402／404 記 `account`，都不重試；離線遇到帳號錯誤（judge 或生成端的 auth／quota／config）整批中止（rc=2、不寫結果檔）。`evaluation` 另記 API 回報的 `judge_model_resp`、`judge_fingerprint`（`system_fingerprint`）、`judge_requests`、`usage`。judge 的描述性校準工具是 `scripts/judge_agreement.py`（歷史 haiku 判定當參考、只描述不判定；只取 2026-09-03 起的抽查，haiku 分數排除 `no_source` 重算）。
...
- lifespan：`assert_single_worker`（偵測到多 worker 拒絕啟動，偵測不到放行）→ `assert_pgvector_version`（低於 0.8 fail-closed，連不上 DB 放行交給 `/healthz`）→ LLM 自檢（`_check_llm_models`：依解析結果檢查 `DEEPSEEK_API_KEY` 有無值、白名單外的模型名（含 `claude-*`）、`ASK_ENABLE_WEB` 開著卻沒有網搜後端；只記 ERROR、不擋啟動：讀取類功能不需要 LLM）→ 背景暖機（embed 再 rerank，**必須依序**，兩執行緒同時首次 import transformers 會競態）。
...
- `/healthz/llm` 回報 DeepSeek 帳號（`app/services/llm_health.py`）：同樣在白名單、只回答本機直連、回應只有 `llm` 一鍵且不含金額。查 `GET /user/balance`，只讀 `LLM_BUDGET_CURRENCY`（CNY）那一筆、依 `currency` 取值不靠索引；低於 `LLM_BALANCE_FLOOR`（70）為 low，缺該幣別或其他幣別非零為 indeterminate。本行程真實請求的 402（`llm_http.last_quota_at`）立刻翻 exhausted，直到一次之後開始的成功查詢。問答主答（`ASK_ANSWER` 任務）解析到白名單外的名稱時不查餘額、回 503 `misconfigured`（每題都會以設定錯誤失敗；其他線上任務都 fail-open，不算）。消費端是探針：`low` 為退出碼 7（WARNING），其餘 503 為 8（CRITICAL，停擺或判斷不出來）；探針兩項 L3 全查、一行帶出全部 reason，退出碼取 8 → 6 → 7 最前面的（8 是最重的故障；7 可能持續到儲值，不能遮蔽 6；5 是 PR-M 退役的 claude 依賴檢查、編號不重用）。狀態只在行程記憶體裡，web 重啟時可能閃一次 RESOLVED（`llm_health` docstring）。
...
- DB 一律 `from app.services.db import SessionFactory`，唯一留在 `db.py` 的 `os.getenv` 是 `REPORT_MARK_DB_URL`。長查詢用 `relax_statement_timeout()`（`SET LOCAL`，因為池化連線的 `SET` 會流到下一個借用者）。`DB_IDLE_TX_TIMEOUT_MS` 預設 0 是刻意的：sync 在交易內呼叫 LLM 標註與嵌入。
...
| LLM 模型 | `LLM_PROVIDER`（唯一合法值 `deepseek`）與 14 個任務旋鈕 `*_MODEL`，一律經 `app/services/llm_models.py` 的 `resolve_model`：非空的任務旋鈕優先，否則查預設表 `DEEPSEEK_DEFAULTS`（網搜那列空字串，其餘 `deepseek-flash`，兩個 judge 自 PR-26/27 起也是，新量尺系譜、門檻數值不變）；空字串視同未設。PR-M 退役的 `claude_cli`／`claude_only`（`RETIRED_PROVIDERS`）與拼錯的值：線上記 ERROR 並當成 `deepseek`，批次與評測的預檢（`scripts/_llm_env.py`）印原始值 rc=2——設了退役值的人多半想讓 LLM 停下來，不能變成照常計費（`/etc/default/report-mark-llm` 缺檔時批次因缺金鑰 rc=2）。網搜沒有後端：`ASK_ENABLE_WEB` 預設 0、前端 `WEB_SEARCH_PAUSED` 隱藏開關，P9 完成後恢復。測試以 conftest 強制 `deepseek`、金鑰空、端點不可達，不打付費 API。白名單 `HTTP_MODELS` 住在這個只依賴標準函式庫的葉模組（`app/config.py` 與 `llm_http` 都 import 它，避免循環）。啟動自檢檢查 `DEEPSEEK_API_KEY` 有無值、白名單外的模型名、網搜總閘，不擋啟動 |
...
| 忠實度 M8 | `ASK_FAITHFULNESS_ENABLED`（1）、`FAITHFULNESS_MIN`（0.9；讀不到時退回舊名 `REPORT_FAITHFULNESS_MIN`，讀者只有監控頁 `_FAITHFULNESS_MIN` 與 `scripts/eval_faithfulness.py`）、`ASK_FAITHFULNESS_SAMPLE_RATE`（1.0）、`FAITHFULNESS_MODEL`（`deepseek-flash`，刻意不沿用 `ASK_INTENT_MODEL`：換路由模型不得靜默換尺；讀分數三處只計現行 judge，見 `app/services/judge_schema.py`）、`FAITHFULNESS_TIMEOUT`（60，目前沒有呼叫端）、`ASK_FAITHFULNESS_TIMEOUT`（每次 judge 呼叫的總期限；未設時 90，依探測延遲以 max(ceil(3×p99), 60) 估算；PR-M 前 Claude CLI judge 是 240）、`ASK_FAITHFULNESS_MAX_INFLIGHT`（2） |
