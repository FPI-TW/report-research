...
五類與 `decided_by`（`precheck`／`overview`／`llm`／`fail_open`／`unknown`）全寫進 `qa_log.filters`；`filters.web` 含 False 也要寫。網搜的逾時只在 `web_on` 時放寬（`ASK_WEB_TIMEOUT`；經 `_with_heartbeat` 驅動時它實際是「第一個輸出（文字或網搜標記）」的期限，之後不設牆鐘上限），免責句由 Python 追加（`WEB_ANSWER_DISCLAIMER`），網搜來源不進 evidence ledger，`drop_abandoned_draft` 只在 `web_on` 時套用。
