...
Python 做所有決定性的事：解析、抽取、切塊、嵌入、儲存、檢索、錨定、聚合、窗期、狀態機、渲染。Claude 只做語意：標註、摘要、問答、研報、訊號擷取、追問、忠實度評審。每個管線階段以檔案 SHA256 `file_hash` 為鍵、可斷點續跑。派生功能（rerank、忠實度、追問、摘錄、換皮、grounding）一律 fail-open 降級，不阻斷主流程。
...
| 檔案 | 責任 |
|---|---|
| `app/config.py` | 集中讀環境變數為 frozen dataclass `Settings`（`get_settings()`）。驗證器對 typo 不靜默：`LOG_LEVEL`、`REPORT_RENDERER`、`EXTRACTOR`、`OBJECT_STORAGE_MODE` 打錯會警告退回預設或拒絕啟動 |
...
| `app/templates/manifest.py` | Typst 模板 registry：`ib-classic`（預設）、`broker-modern`、`privatebank-dark`。未知 id 退回預設 |
...
| `retrieval_pipeline.py` | 問答與研報的**唯一檢索入口**：embed → `hybrid_search` → rerank（fail-open）→ `build_context`；`retrieve_context_multi` 做子查詢扇出與合併 |
...
| `textnorm.py` | `clean_text`（折換行，檢索片段用）、`clean_extracted`（顯示用）、`norm_for_match`（與 DB 生成欄 `content_norm` 逐字等價）、`soft_break_token` |
...
| `query_planner.py` | 子查詢分解（問答與研報共用 profile）；禁止 import `answer`／`retrieval_pipeline`／`report` |
| `faithfulness.py` | 主張拆解與 grounding；`is_numeric_claim` 是問答抽查與研報修正輪的唯一閘門 |
| `evidence.py` | 證據帳本：引用存在不等於真實性 |
| `followups.py` | Haiku 產至多三條追問，fail-open 回空 |
| `report_gate.py` | 是否建議出研報，純規則（引用篇數或答案長度） |
...
### 2.3 `app/services/` 深度研報

| 檔案 | 責任 |
|---|---|
| `report.py` | 研報生成編排（逐節主軌、單次退路）、PDF 落地、rendition、持久化 |
| `report_writer.py` | M7 大綱 → 逐節狀態機；純資料層不接 LLM 的部分與內容層同檔 |
| `typst_render.py` | markdown → 型別化文件模型 → Typst → PDF；`_tstr` 安全邊界 |
| `pdf.py` | markdown → HTML → WeasyPrint；文字正規化、免責、`inject_kpi`／`inject_charts`，兩軌共用 |
| `chart.py` | 圖規格 → inline SVG，純函式零 I/O |

### 2.4 `app/services/` 讀取零 LLM 的功能
...
| `web/routers/` | 12 支 router：`ask`、`report`、`search`、`qa_history`、`monitor`、`radar`、`reading`、`report_file`、`health`、`auth_pages`、`spa`、`brief`。全部 `APIRouter()` 不帶 prefix（`tests/test_docs_contract.py` 靠這個抓完整路徑） |
| `web/deps.py` | 跨 router 共用符號與測試 patch 的單一位置；`_sse`、心跳 |
| `web/report_runs.py` | 研報背景 run 登錄表、重播緩衝、訂閱、取消、對話刪除租約 |
...
唯一一組刻意的循環：`retrieval_pipeline.py` **頂層** import `answer`（取 `Source` 與 `build_context`）；反方向（`answer` → `retrieval_pipeline`、`answer` → `agentic_qa`、`agentic_qa` → `retrieval_pipeline`、`answer` → `report`）一律函式內 import。`query_planner` 與 `agentic_qa` 的 docstring 有明文 import 約束。

其他函式內 import 是載入成本考量而非循環：`report.py` 延遲 import `typst_render`、`pdf.py` 延遲 import weasyprint、`extract.py` 延遲 import pdfplumber 鏈、`object_storage.py` 只在啟用 R2 時 import boto3、`extraction/__init__.py` 刻意不 import 子模組。
...
- 選篇分**兩條互不共用**：檢索頁走 `retrieval.rank_reports`（`relevance`／`date_desc`／`date_asc`，相關度以 `BAND_WIDTH=0.05` 分帶再看日期），消費端 `web/routers/search.py`；問答與研報走 `answer.select_reports`（聚合 → 排序 → 過舊軟截斷 → 相關度下限 → 過舊配額 → 字數預算，MMR 分支整段 fail-open）。調問答新近度改 `rank_reports` 沒有作用。
- `retrieval_pipeline.retrieve_context`：短連線做檢索，rerank 前拍 `gate_scores` 快照；rerank 未實際套用時 `gate_scores` 不傳。`retrieve_context_multi`：逐子查詢 `hybrid_search`（受 `REPORT_FANOUT_CONCURRENCY` 控）→ `merge_scored` 同 chunk 取最大 → `_retier_to_question` 以原題重算 tier → 合併後單次 rerank。多查詢且 rerank 未套用時整批降級回原題單查詢。
...
事件序（services 層 yield 的 kind）：`status`（stage：`understanding`、`retrieved`、`generating`、`reading`、`searching_web`、`evaluating`）、`sources`、`token`、`notice`、`ext_sources`、`followups`、`done`（`cited`、`qa_id`、`conversation_id`、`thinking_ms`、`offer_report`、`report_title`、`root_qa_id`、`version_count`；簡→繁有變動時多帶 `answer`；婉拒版帶 `notice_kind` 且無 `qa_id`）。web 層補 `queued` 與 `error`。契約在 `tests/fixtures/sse_events.json`。
...
簡繁守門 `zh_hant.to_traditional` 的六個寫入點：`answer.py` 三處（overview 回答、網搜答案、主答案收斂）、`report.py` 兩處（逐節與單次 markdown）、`signal_extract.py` 一處（訊號 summary）。串流路徑刻意不中途轉。**逐字引文不轉**：`report_takeaway.quote`、`thesis_dimensions[*].evidence`、`full_text`、`report_chunk.content`、`report_section.draft_markdown`。

## 6. 深度研報

- 生成跑背景任務（`web/report_runs.py`），`POST /api/report` 只是訂閱端。登錄表 `_RUNS`／`_BY_KEY` 是行程內狀態、重啟即滅；去重鍵 `sha256(question, conversation_id, template_id, locale)` 與 `report_writer.synthesize_request_key`（`template_id` 刻意不進鍵、`locale` 必須進鍵）是兩把不同的鍵。`report_writer.open_run` 把久無心跳（`REPORT_RUN_STALE_SECONDS`，預設 1800）的 in-flight run 視為可重試，兩者成對。`_REPORT_GATE` 由背景任務持有、不掛在 handler 上；接回既有 run 不佔名額。
- 重播規則：`_VOLATILE = {"token"}` 不進緩衝；`_SLIM_KEEP` 讓 `section_draft` 只留 `position`、`section_key`、`heading`，`document_revision` 只留 `revision_id`、`revision`；`queued` 原樣重播。新增事件或欄位要同步改 `tests/fixtures/sse_events.json` 與 `frontend/src/lib/askSchemas.ts`，漏了會「直播看得到、重連看不到」。`cancel()` 是唯一取消手段；`shutdown()` 等待收尾，否則留 in-flight run 擋冪等鍵 30 分鐘。
- 狀態機（`report_writer.py`，與 `db/schema.sql` CHECK 逐字對齊）：run `queued → retrieving → outlining → drafting → verifying → rendering → completed`，`failed`／`cancelled` 恆可進；允許向前跳階，因為 `_audit` 對每次稽核寫入 fail-open，只准單步會讓一次 DB 抖動毒化整條鏈。section `pending → retrieving → drafting → drafted → verifying → final`／`failed`。
- 逐節流程（M7，預設）：`plan_outline`（五章骨架固定，LLM 只決定 `analysis` 子節，`section_coverage` 分母恆為 5）→ 逐節 `retrieve_for_section` 與串流草稿 → grounding → `_section_needs_fix` 只在數值主張支持率低於 `REPORT_FAITHFULNESS_MIN` 時觸發修正一輪（`degraded` 不修；修正輪有自己的預算閘 `plan_fix_action`）→ `assemble_final`（引用標記異常或編號異常則 `failed`）→ `document_revision` → `__final__`。單次路徑是退路，`produced`（是否已 yield 過 token）是退單次的硬邊界；退單次時先送空 `outline`，前端據此改看不定量進度。
- 預算：`REPORT_PLANNER_TIMEOUT` ＋ `REPORT_RETRIEVE_BUDGET` ＋ `REPORT_DRAFT_BUDGET` ＋ `REPORT_SECTION_WALL` ＋ 收尾；`REPORT_FINALIZE_RESERVE` 保留給重生與 grounding。保底集合是三個骨架節加第一個 analysis 子節。
- 渲染雙軌：`REPORT_RENDERER=typst` 為主，任何例外 fail-open 回退 WeasyPrint（`strict_renderer` 時改拋 `PersistedRendererUnavailableError`）；出事時設 `REPORT_RENDERER=weasyprint` 全域回退。**兩軌都跑過 `pdf.py` 的文字正規化與免責**（`report_disclaimer(locale)` 同源、不依賴 LLM）。模板契約是五個匯出函式 `report`、`section-heading`、`kpi-strip`、`chart-figure`、`refs-block`，以 `typst_render.emit_typst` 的 `#import` 行為準；少一個只會靜默退化成 WeasyPrint。「編譯成功」不是驗收，頁數與逐頁檢視才是。
- 安全邊界：LLM 原文永不直接拼進 Typst 原始碼。只有 pandoc 跳脫後的片段與 JSON 驗證過的型別化資料能通過，其餘經 `_tstr()`（反斜線先跳脫，再引號、換行）。章節標題是唯一未經 pandoc 的 LLM 原文，已用 `_tstr` 封死。編譯 root 是隔離暫存目錄，`_strip_images` 在 AST 階段拒絕檔案路徑。pandoc reader 用 `gfm-tex_math_dollars`，否則財經文本的美元符號被當數學式。
- `pdf.py` 的 `inject_kpi`／`inject_charts` 是最後一道且沒被 try 包住：JSON 壞、`items` 非 list、單項非 dict 一律移除該塊並 warning；KPI 最多 5 筆（與 `typst_render._KPI_MAX_ITEMS` 一致）。版面問題走 Python 決定性後處理（`split_long_paragraphs`、`soft_break_token`），不改 prompt，四份研報 prompt 是平行手抄副本。
...
- lifespan：`assert_single_worker`（偵測到多 worker 拒絕啟動，偵測不到放行）→ `assert_pgvector_version`（低於 0.8 fail-closed，連不上 DB 放行交給 `/healthz`）→ 背景暖機（embed 再 rerank，**必須依序**，兩執行緒同時首次 import transformers 會競態）。收工 `report_runs.shutdown()`。
- 併發閘：`/api/ask` 容量 3 寫死在 `web/routers/ask.py` 的 `_ASK_GATE`（佇列 `ASK_MAX_QUEUE`，滿載 429 ＋ `Retry-After: 30`）；`/api/report` 容量 `REPORT_SEMAPHORE`（佇列 `REPORT_MAX_QUEUE`，429 ＋ `Retry-After: 120`）。都是 per-process，lifespan 擋多 worker。
...
schema 名 `research`，11 張表（`db/schema.sql`），沒有 migration 工具，冪等只涵蓋 `ADD COLUMN IF NOT EXISTS`；改 CHECK 約束在既有庫是 no-op，要另寫 `ALTER`，`db/expected_constraints.txt` 由 `tests/test_schema_constraints.py` 對帳。
...
| `report_doc` | 研報成品：`markdown` 為真相、`pdf_path`／`pdf_object_key`、`outline`、`claim_evidence`、`template_id`、`renderer` | `qa_id`、`report_run_id` 軟連結 |
| `report_run`、`report_section` | 逐節生成狀態機、`request_key` UNIQUE、`checkpoint` | section FK → run CASCADE；對 `research_report` 刻意無 FK |
| `report_rendition` | 換模板重排的每一版 PDF | `report_id` 軟連結 |
...
備份只涵蓋七張不可重建的表（`qa_log`、`report_doc`、`report_rendition`、`report_takeaway`、`report_signal`、`report_run`、`report_section`）→ NAS；語料層刻意不備。
...
| M5／M6 規劃 | `QA_PLANNER_MODEL`、`QA_PLANNER_TIMEOUT`（45，冷啟動 ttft 約 10 秒）、`QA_PLANNER_MAX_SUBQUERIES`（3）、`QA_MAX_ROUNDS`（2）、`QA_AGENTIC_ENABLED`（1）、`QA_AGENTIC_TIMEOUT`（90）、`QA_SUBQUERY_MAX_REPORTS`（5）、`REPORT_PLANNER_MODEL`、`REPORT_PLANNER_TIMEOUT`（30）、`REPORT_PLANNER_MAX_SUBQUERIES`（8）、`REPORT_FANOUT_CONCURRENCY`（3）、`REPORT_SUBQUERY_DENSE_SCAN`（200）、`REPORT_TOTAL_CANDIDATES`（600）、`REPORT_MMR_ENABLED`（1）、`REPORT_MMR_LAMBDA`（0.7）、`REPORT_MMR_MAX_PER_SOURCE`（6）、`REPORT_MMR_MAX_PER_MONTH`（0，刻意關） |
| 研報 | `REPORT_MODEL`（claude-sonnet-5）、`REPORT_DEEP_K`（30）、`REPORT_MAX_REPORTS`（25）、`REPORT_MAX_PASSAGES`（6）、`REPORT_MAX_CONTEXT_CHARS`（40000）、`REPORT_TIMEOUT`（600）、`REPORTS_DIR`（data/reports）、`REPORT_RENDERER`（typst）、`REPORT_ENABLE_WEB`（1）、`REPORT_THIN_COVERAGE`（8）、`REPORT_MIN_CITED`（3）、`REPORT_LONG_ANSWER_CHARS`（400） |
| 逐節 M7 | `REPORT_SECTIONED_ENABLED`（1）、`REPORT_OUTLINE_TIMEOUT`（45）、`REPORT_OUTLINE_MAX_SUBSECTIONS`（5）、`REPORT_SECTION_TIMEOUT`（150）、`REPORT_SECTION_MAX_REPORTS`（8）、`REPORT_SECTION_MAX_PASSAGES`（4）、`REPORT_SECTION_MAX_CONTEXT_CHARS`（12000）、`REPORT_SECTION_RERANK_CANDIDATES`（40）、`REPORT_SECTION_RETRY`（1）、`REPORT_SECTION_THIN_COVERAGE`（3） |
| 逐節預算 | `REPORT_BUDGET_LOOKAHEAD_ENABLED`（1）、`REPORT_DRAFT_BUDGET`（900）、`REPORT_SECTION_WALL`（240）、`REPORT_FINALIZE_RESERVE`（180）、`REPORT_RETRIEVE_BUDGET`（300）、`REPORT_RUN_STALE_SECONDS`（1800） |
| rerank | `ASK_RERANK_ENABLED`（1）、`ASK_RERANK_CANDIDATES`（50）、`ASK_RERANK_TIMEOUT`（60；實測 50 對約 34 秒）、`REPORT_RERANK_ENABLED`（1）、`REPORT_RERANK_CANDIDATES`（120）、`REPORT_RERANK_TIMEOUT`（180）、`RERANK_MODEL` |
| 忠實度 M8 | `REPORT_FAITHFULNESS_ENABLED`（1）、`ASK_FAITHFULNESS_ENABLED`（1）、`REPORT_FAITHFULNESS_MIN`（0.9）、`ASK_FAITHFULNESS_SAMPLE_RATE`（1.0）、`FAITHFULNESS_MODEL`、`FAITHFULNESS_TIMEOUT`（60）、`ASK_FAITHFULNESS_TIMEOUT`（240，實測 48–142 秒）、`ASK_FAITHFULNESS_MAX_INFLIGHT`（2） |
| 抽取與儲存 | `EXTRACTOR`（pypdf）、`EXTRACTION_REVIEW_MIN`（0.6）、`OBJECT_STORAGE_MODE`（local）、`R2_ENDPOINT_URL`、`R2_BUCKET`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY`、`R2_PRESIGN_TTL_SECONDS`（3600，上限一小時） |
...
既有散在各檔的 `os.getenv` **不要順手搬**（`grep -rn os.getenv app web`）：`REPORT_MARK_DB_URL`、`ASK_FOLLOWUP_MODEL`、`ASK_FOLLOWUP_TIMEOUT`、`REPORT_MARK_RERANK_WORKERS`（3）、`REPORT_MARK_RERANK_TIMEOUT`（30）、`REPORT_RUN_RETENTION_SECONDS`（600）、`REPORT_SEMAPHORE`（1）、`REPORT_MAX_QUEUE`（5）、`ASK_MAX_QUEUE`（20）、`SSE_HEARTBEAT_INTERVAL`（20）、`SKIP_WARMUP`、`DEV_NO_AUTH`、`REPORT_MARK_MAX_TRACKED_FAIL_IPS`（4096）與 auth 那組 `REPORT_MARK_*`。`REPORT_MARK_*` 前綴只給 auth／DB，帶前綴的例外是 live 的不要改名。

DB 連線數算式（`.env.example`）：`worker 數 × (DB_POOL_SIZE + DB_MAX_OVERFLOW) + 批次腳本數 × 2` 要小於 97；現況 1 × 20 + 3 × 2 = 26。加 `--workers`、提高 `REPORT_SEMAPHORE`、放寬 `_ASK_GATE` 之前要重算。
...
| `ASK_*` 逾時、`REPORT_*` 預算、`DB_*` 逾時的數字 | `app/config.py` 逐條註解 |
| `zh_hant.py` 的判別法與門檻、`faithfulness.is_numeric_claim`、`reading/queries.py` 的 `_SIMILAR_SQL`、`web/report_runs.py` | 各檔開頭實測紀錄 |
