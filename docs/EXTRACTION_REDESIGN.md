...
**效力邊界先讀**：15 份全部是 `annotation_status = draft`——順序層與評等／目標價是模型看頁面渲染圖＋ pdfplumber 逐行文字草標的，**尚未人工覆核**。數字是暫定值；覆核後改 `reviewed` 重跑，`eval_compare` 會因 `n_reviewed` 變動判為不可比，那是預期行為。

| 指標 | pypdf 6.12.2（現況） | pdfplumber spike（`ext-2026-08-31.v1`，唯讀並排） | E1 | E4 |
|---|---|---|---|---|
| 閱讀順序：關鍵句命中率（全體） | **1.000** | 0.962（漏 7 句，見下） | | |
| 閱讀順序：相對順序正確率（全體配對） | **0.805** | 0.875 | | |
| 閱讀順序：相對順序正確率（**同一文字流內**配對） | **0.932** | 0.986 | | |
| 閱讀順序：相對順序正確率（雙欄子集，13 檔） | 0.798 / 同流 0.938 | 0.867 / 同流 0.983 | | |
| 各券商：kgi（4） | 0.758 / 0.964 | 0.895 / 0.935 | | |
| 各券商：masterlink（4） | 0.972 / 1.000 | 0.873 / 1.000 | | |
| 各券商：sinopac（3） | 0.816 / 0.880 | 0.833 / 1.000 | | |
| 各券商：yuanta（3） | 0.726 / 0.919 | 0.938 / 1.000 | | |
| 各券商：goldman_sachs（1） | **0.321** / 0.731 | 0.833 / 1.000 | | |
| 目標價 coverage（現況 `report_signal`） | 0.680（17/25） | 同左（欄位層與抽取器無關） | | |
| 目標價 accuracy | 1.000（17/17） | 同左 | | |
| 評等 coverage / accuracy | 0.545 / 1.000 | 同左 | | |
| 幻覺率（evidence 錨不回比例，`prefix` 計為錨不回） | 0.059（1/17） | 0.176（3/17）＊ | | |
| 文字召回率（健檢：去空白字元合計） | 260,611 | 264,424 | | |
...
1. **現況的問題不是「抽不到」，是「順序」**——184 句 pypdf 全部命中，但配對正確率只有 0.805；即使不計跨欄慣例、只看同一文字流內部，仍有 6.8% 的句對顛倒。最差的是高盛雙欄（0.321）與凱基 daily story（華通 0.545）。這與 §1.2 「字元召回率量了不會動、主指標要換成順序」的判斷一致。
2. **pdfplumber 把同流配對從 0.932 拉到 0.986、高盛從 0.321 拉到 0.833**，但**不是全面更好**：masterlink 全體配對從 0.972 掉到 0.873（側欄與主文的相對位置變了），且 7 句漏抽——`台股一週大勢分析`／`公司拜訪快報` 是被 `header` 分類吃掉的真標題，元大早報 p1 左欄清單與高盛的目標價行是被表格偵測收走、序列化成 markdown 後加了 `|`。這正是 §9 第 3 步「不同不等於更好」要人工看的東西，也是進 `E1` 前要修的兩個分類問題。
