...
- **時效型金融問題**（即時／最新報價、財報、公告、利率、政策）→ 受信任資料 adapter 優先，答案必須顯示「截至時間」與外部來源；不得以舊研報片段充當即時資料。adapter 不可用時：使用者**未**開啟網搜 → 安全婉拒（M4 既有行為）；使用者**已明確開啟**網搜（M11）→ 以網路公開資訊作答，但必須附固定免責、標明每項數值的資料時間，且該免責由 Python 追加而非交給 prompt。
...
> **M11 修訂（2026-08-20）**：問答的一般 WebSearch 由「一律不允許」改為**使用者每題自選**（`/api/ask` 的 `web` 欄位，前端預設關閉，伺服器另有總閘 `ASK_ENABLE_WEB`）。信任邊界一步都沒有放寬——網搜結果**仍然不進 evidence ledger**（`manifest_from_answer` 對缺 `snapshot_ref`／`content_hash` 的來源逐筆跳過），仍然不得冒充受信任行情資料，時效題也仍以 adapter 優先。改變的只是「查不到就婉拒」與「查不到就用公開資訊回答並標明它是公開資訊」之間的選擇權歸誰。
...
| 一般 WebSearch／模型自由瀏覽 | 使用者開啟時允許（M11） | 使用者開啟且 adapter 不可用時允許（M11） | 允許（`REPORT_ENABLE_WEB`） | 不得繞過上述 adapter；不得偽裝成受信任資料 |
