...
>
> **文件狀態（2026-07-13）**：共用檢索 helper 與 reranker 已接入；重新生成、停止、編輯重送與追問的資料模型與 API 細節以 `docs/superpowers/specs/2026-07-13-m3-qa-ux-design.md` 為準。本文負責長期架構與產品決策，避免與 M3 規格重複定義。
...
改寫 `intent.py` 的語義：從「離題就擋」變成「問題類型 + 工具政策」分流，維持金融知識問題的 fail-open，但不以 fail-open 代替時效與投資風險控管。
...
- **時效型金融問題**（即時／最新報價、財報、公告、利率、政策）→ 走受信任資料或網搜政策，答案必須顯示「截至時間」與外部來源；不得以舊研報片段充當即時資料。
...
> 產出：`scope_router` 回傳 `{off_topic | overview | corpus_qa | time_sensitive | advice_risk}`，並同時給出 `tool_policy`（只用語料／可用受信任外部資料／不提供個人化指令）。首輪與續問必須共用同一判準，防止「首輪 IN、續問 OUT」漂移。
...
1. **規劃步**：Claude（Haiku）看問題（＋多輪脈絡與 `tool_policy`）→ 輸出 1–N 個子查詢、面向與資料新鮮度需求；除了 overview／純介面操作外，不可決定「免檢索直接答」。
2. **檢索步**：Python 執行 `hybrid_search` + **BGE-reranker**（共用研報藍圖 Phase 1 的 `rerank.py`）。
3. **評估步**：Claude 看目前證據 → 決定「足夠、作答」／「再查一輪（補面向）」／「依 `tool_policy` 補受信任外部資料」。Python 對每輪設定子查詢數、候選數、總逾時與取消傳播，避免三個 ask 併發時再放大為大量 CLI／DB 工作。
4. **作答步**：綜合證據、串流輸出；內部用共用 `evidence_id`，最後才渲染研報 `[n]` 或外部來源標示。外部論點必須顯示來源性質與截至時間，並保存 URL、取得時間與內容雜湊，不與研報引用混用。
...
| 即時報價／公告 | 受信任外部資料 + 截至時間 + 來源標示 | 依資料來源；不可用舊研報冒充即時資料 |
| 一般綜合題 | agentic 1–2 輪 | 現況的 ~1.5–2.5×（可接受，仍是「聊天級」） |
| 需搜網補充 | +1 輪網搜 | 視 WebSearch 而定，串流「搜尋網路中」遮蔽感知 |

手段：規劃/評估用 **Haiku**（快）、檢索並行、rerank 控候選上限、每請求的子查詢／外部搜尋／總逾時預算、快速路徑豁免、串流讓**首 token 感知延遲**低。硬上限 `QA_MAX_ROUNDS=2`，取消請求必須取消尚未開始的規劃／檢索工作。
...
app/
  services/
    ...
    evidence.py         # 共用證據帳本、來源持久化、最後引用渲染
    # 共用自研報藍圖：
...
| 語料外作答導致幻覺或來源不可追溯 | tool policy 限制外部來源；持久化 URL、來源性質、發布／取得時間與內容雜湊；外部論點不混入研報引用 |
| 把過期研報當即時資料 | `time_sensitive` 路由強制受信任外部資料與截至時間；無可用資料就明確說明 |
...
2. **範圍路由**：`intent.py` → 問題類型 + tool policy，加入時效型與個人化建議保護；首輪／續問共用判準。
3. **共用地基接入**：把研報藍圖 Phase 0/1（型別化 row、`retrieval_pipeline`、`rerank`）接進問答的單輪路徑，先拿到 rerank 的精準度增益。
4. **Agentic 迴圈**：導入 `agentic_qa.py` 受控多輪 + 有新鮮度保護的快速路徑；用凍結問答題組驗證忠實度、精準度與延遲分佈。
