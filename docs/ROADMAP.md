...
| **生產韌性** | DB 自動重啟、免認證 `/healthz`、`OnFailure` 告警、systemd unit 收回 repo；DB 每日備份至 NAS（七張不可重建的表，已做過還原演練）。**批次停更偵測已移除**——best-effort 的批次連續失敗仍然不會有任何訊號 | `web/routers/health.py`、`deploy/systemd/`、`scripts/db_backup.sh`、`docs/production_resilience.md` |
