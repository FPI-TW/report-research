...
| **CI 與分支保護** | 三個 job：前端（ESLint＋tsc＋vite build＋vitest）、後端（ruff＋pytest）、schema 契約（pgvector container）；必要檢查仍是前兩個 | `.github/workflows/ci.yml`、`db/expected_constraints.txt` |
