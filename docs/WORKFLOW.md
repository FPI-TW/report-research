...
| 症狀 | 看哪裡 |
|---|---|
| 問答每題回「帳號異常」／「設定有誤」 | `/healthz/llm`（`scripts/check_web_health.sh` rc=7／8）與 web 日誌的啟動自檢（金鑰、白名單外的模型名、`ASK_ENABLE_WEB`）；處置見 `docs/production_resilience.md` |
...
| sync 或各 oneshot 起不來 | `/etc/default/report-mark-sync` 的 `SYNC_PATH_EXTRA` 是否含 uv 所在的 `~/.local/bin` |
| 批次 rc=75 | 撞批次鎖（`scripts/_claude_lock.py`），不是錯誤；`make freshness` 判是否停更 |
