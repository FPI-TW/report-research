...
## 後續：導入速率整併（2026-06-23）

為讓三個處理面板版型一致，把「導入速率」從**頂部獨立 tile** 移除，改在 **INGEST 面板底部**以 `.psub` 行顯示（與標註／摘要對齊）：

- 頂部 tiles 由 5 個減為 4 個（已導入報告／總片段／標註進度／摘要進度）。
- INGEST 面板大字由「速率」改為**本輪導入數**（`ingest.ingested`），側欄留「失敗」，底部 psub 顯示速率。
- 導入無已知總量，故不給 ETA；第二段改放每秒片段數（cps），保留原 tile 的「片段/秒」資訊。新增純函式 `ingestRateText(rpm, cps)`（eta.js）→「速率 X 篇/分 · Y 片段/秒」，含單元測試。
- 移除 `tick()` 中 `t-rate`／`t-rate-d` 與 INGEST 大字速率 tween。

## 不做（YAGNI）

- 不改後端、不新增 API 欄位（前端已有足夠資料）。
- 不做歷史曲線/圖表，只給目前速率與 ETA。
- `rate()`（rpm/cps/spm/tpm 計算）維持 inline，純文字格式器才抽出可測。
