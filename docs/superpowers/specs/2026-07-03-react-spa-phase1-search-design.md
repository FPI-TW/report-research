...
| `TableView.tsx` | 表格（欄：標題/市場/類型/來源/日期；表頭可排序＝呈現態 `tableSort`） |
...
- `ReportDetailModal.tsx`（檢索用）：資料**取自結果 item**（標題/市場/來源/日期/類型/商品/摘要，**不再打 API**）＋PDF iframe `/api/report/{id}/file`＋`在新分頁開啟`／`下載原始檔`（同一端點）；下載連結 scheme 守門（僅 `/` 或 `https?:`）。
...
- **分頁（手動累積）**：state 持有 `pages`（已載入頁）與 `loaded`（已載入筆數）；`載入更多` 以 `offset=loaded` 抓下一頁並 append；`hasMore = loaded < total`；`remaining = total - loaded`。初始 `limit=8`，每次 `+8`（`SEARCH_PAGE_SIZE`，可調常數）。換 query key（q/篩選/sort）→ 重置累積。
...
- `ReportDetailModal`：入參為結果 item（不打 API）；顯示 metadata（標題/市場徽章/來源/日期/類型/商品/摘要）＋PDF iframe `src="/api/report/{report_id}/file"`＋`在新分頁開啟`(`<a target="_blank" rel="noopener">`)／`下載原始檔`(`<a download>`)。連結 scheme 僅允許相對 `/` 或 `https?:`。
...
## 10. 測試
- **純函式（Vitest，RED→GREEN）**：`filters`(參數↔URL 雙向、單值、預設不帶)、`grouping`(YYYY-MM 分組、無日期置底、組內序沿排序)、`highlight`/`terms`(分段、無 term 原樣、特殊字元不炸)、`isLatest`(嚴格大於、同日第一筆、無日期不參與、載入更多重算)、`tableSort`(各欄升降序)、`resultsMeta`(三種文案分支)。
- **元件（Testing Library）**：SearchBar 送出＋IME 守衛；MarketChipBar active＋count；MoreFiltersPopover 單選切換＋badge 計數＋清除；ActiveChips 可刪；ResultCard 兩模式（高亮/相關度條 vs 摘要）；ViewSwitch 切換**不重抓**（mock fetch 呼叫次數不增）；LoadMore 累積；EmptyState；`Modal` 開關/Esc/focus-trap；`ReportDetailModal` iframe src + 下載 scheme 守門。
- **e2e 冒煙（`:8098`，Playwright）**：登入 → `/app/search` browse 載入 → 輸入關鍵字搜尋 → 套用一個篩選 → 月分組顯示 → 切表格檢視（不重抓）→ 載入更多 → 開 `ReportDetailModal`。
...
| `GET /api/report/{id}/file` | 原始檔 | PDF（iframe/下載） |
| `GET /api/stats` | 市場 count | `markets[{market,count}]`（Phase 0 `useStats`） |
