...
### 2.1 設計稿與資料能力的落差（2026-07-17 實查）

Claude Design 定稿（專案 `1e63e91d-fd04-4cc9-ae2a-b5be907d546b`）提出了「共識目標價光譜」
（beeswarm 散點＋IQR 盒＋中位／現價基準線）、報頭 ident（含現價）、本期變化帶與共識分歧
數字。**這些在現有資料下無法渲染**，故延後（D9）：

| 設計稿需要 | 實際（本機＝生產 DB） |
|---|---|
| 12 家券商目標價分布 | **最多 5 家**（3008）；設計稿主角 2330 只有 **3 家** |
| 現價 | **無任何來源**——`app/services/findb_client.py` 不存在，DB 無 price/close 欄位 |
| 12 家評等分布（8買/3持/1賣） | 評等家數最多亦為 5 家 |
| EPS vs 90 天前、本期 vs 前次 | 需 signal 時序；現有 359 筆訊號、98 筆有目標價、26 個標的 |

根因有二，且互相獨立：

1. **`report_signal` 覆蓋率 0.71%**（104/14,567 篇），`extract_signals` 只跑過語料子集；
   補完約需 170 小時（5,094 份 × Sonnet 100–135s）。
2. **findb 未整合**——Roadmap Phase 2 的 `findb_client.py`（唯讀 Serve API 取行情）尚未實作。

更根本的錯配：那是一份**個股研報**版型（標的識別／現價／目標價分布／本期變化），但 M7
生成的是**主題式研究**（五章骨架、可跨標的跨市場、內容為多篇報告的文字彙整），markdown
裡本就沒有這些結構化數字。

**設計稿仍可用的部分全數採用**：雙欄版型、排版規範（§5.1）、KPI 帶版型（接 ```kpi）、
引用、免責、方法論註記、`cite` 改純數字（不用金色）。

未來要讓光譜有意義，需要同時滿足：findb 現價 + signal 覆蓋率上來 + 研報知道自己在講哪個
標的（例如雷達頁「產生此標的研報」入口帶 `market/code`）。
...
| **D8** | **採用設計稿的排版規範（見 §5.1）** | 使用者於 Claude Design 定稿時新增。模板規則，避免自動彙整時過度強調。 |
| **D9** | **資料驅動元件（目標價光譜／報頭 ident／本期變化帶／共識分歧數字）延後，不進 M9a** | 使用者拍板。設計稿假設個股研報的資料密度，實際資料給不出來（見 §2.1）。M9a 只落地能用真資料渲染的部分。 |

### 5.1 排版規範（模板規則）

- **粗體節制**：每段最多 1–2 個粗體，優先給「投資結論／核心數字／關鍵變化」；同一行避免
  3 個以上粗體片語；勿同時粗體結論、原因、方向與區間。
- **金色節制**：金色只用於品牌豎線（章節規線／副標左線）、圖表中位數與核心強調；
  **一般資訊（引用編號、次要標籤）不用金色**。
- **色彩可列印**：正負面不可只靠顏色，需搭配 `+/−`、`▲/▼` 或文字；語意綠／紅採低飽和，
  並通過黑白列印測試。
- **字級與對齊**：正文約 10.5px、註腳／圖例約 8px 起，**不以縮小字級處理溢出**；數字一律
  `tabular-nums`；KPI 卡內距一致、主數字與來源列基線對齊；引用編號用固定 tab stop。
