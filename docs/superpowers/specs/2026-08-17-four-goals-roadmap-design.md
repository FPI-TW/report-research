...
本文件初版據此推估首 token 牆鐘 ≈ 55–60 秒（檢索 48s ＋ CLI 冷啟動 10s）。**那個推估錯了，見下。**

#### 已量測（2026-08-17，`scripts/measure_baseline.py --days 365`，n=91）

| 指標 | p50 | p95 | p99 | max |
|---|---|---|---|---|
| `latency_ms`（整題完成，n=88） | **41.6s** | 119.0s | 182.6s | 182.6s |
| `thinking_ms`（首 token 前，n=84） | **23.1s** | **94.6s** | 167.6s | 167.6s |

**三個結論改變了目標 1 的形狀：**

1. **中位數遠優於推估，但尾巴極長。** 首 token 中位數 23.1s，不是推估的 55–60s；但 p95 是 94.6s，是中位數的四倍。**問題不是「每題都慢」，是「分佈極度發散」**——優化目標應該是壓尾巴，不是壓中位數。這與「砍 rerank 候選數」這類全域減量手段的方向不一致：它把中位數與尾巴一起下拉，卻要全體付品質。
2. **`p99 == max`（182,556 = 182,556）。** 即使 n=88，p99 仍等於單一最大值——樣本量對尾端統計仍然不足。任何「p99 改善了」的宣稱在這個樣本量下都不成立，**只有 p50 與 p95 可引用**。
3. **窗期必須是 365 天。** 全年只有 91 題（約 0.25 題／天，低於先前引用的 0.7）。30 天窗期會退化到約 21 列，p95 與 p99 會一起塌到 max。

#### 路由遙測：目前無法回答分類器失敗率

91 列中 **76 列完全沒有 `path`／`decided_by`**（M4 之前的舊列）。有遙測的 15 列為 `corpus_qa/llm` 8、`overview` 5、`time_sensitive` 2，**`fail_open` 為 0**——但 n=15 太小，不足以判定分類器失敗率。這個數字要靠往後累積，不是現在能回答的。

#### 仍未量測的缺口

1. ~~`qa_log.thinking_ms` / `latency_ms` 的實際分佈從未統計過。~~ **已於 2026-08-17 量測，見上表。** 背景仍值得記錄：這兩欄是 `_log_qa` 的 INSERT 參數（`app/services/answer.py:959-990`，`:lat`／`:think`），只要那次寫入執行就落欄，與 `logger.info` 有沒有印出無關；`db/schema.sql` 對 `research.qa_log` 未設 TTL 或分區。**可撈窗口是表齡，不是 log 齡。** 真正受 2026-07-29 `logging_setup.py` 修復所限的是 `qa_timing` 這條 **log 本身**——`dense_ms`／`lex_ms`／`route_wait` 等分段遙測只落 log、不落欄位，這些數字的可撈窗口才約一個月。
...
### 4.3 資料覆蓋率基準：目標 3 需要 —— **已量測，結論是可行**

2026-08-17 實測（`scripts/measure_baseline.py`，母體 `is_research` 共 14,986 篇）：

| 欄位 | 非空篇數 | 覆蓋率 |
|---|---|---|
| `stock_targets`（陣列，內文抽取） | 10,919 | **72.86%** |
| `stock_code`（純量，檔名／標註） | 2,828 | 18.87% |
| 兩者聯集 | 10,935 | **72.97%** |

**這解除了目標 3 最大的未知數。** 先前的擔憂是「`stock_targets` 由標註批次抽取，覆蓋率太低會讓標的優先那一路經常是空的」——72.9% 遠高於可用門檻，標的路召回是實質有效的，不是裝飾。

兩個附帶發現：

- **`stock_code` 只有 18.87%，不能單獨當召回依據。** 聯集只比 `stock_targets` 多 16 篇，代表 `stock_code` 幾乎完全被 `stock_targets` 涵蓋。目標 3 的 SQL 仍應兩欄都查（成本為零、`idx_rr_stock_code` 現成），但期望值來自 `stock_targets`。
- 剩下的 27% 沒有任何標的欄位——那一段只能靠全文比對命中，正是「標的優先、全文次位」這個設計要涵蓋的部分。
