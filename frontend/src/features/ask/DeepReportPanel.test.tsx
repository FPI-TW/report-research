...
test('generating 顯示進度與階段文字', () => {
  render(<DeepReportPanel report={rs({ status: 'generating', stage: 'writing', startedAt: Date.now() })} onGenerate={() => {}} onDecline={() => {}} />)
...
test('done 顯示下載連結（scheme 守門相對路徑）', () => {
  render(<DeepReportPanel report={rs({ status: 'done', downloadUrl: '/api/report-doc/rp1/pdf', title: 'T' })} onGenerate={() => {}} onDecline={() => {}} />)
...
test('done：協定相對/跨源 downloadUrl 不渲染下載連結（scheme 守門）', () => {
  const { rerender } = render(<DeepReportPanel report={rs({ status: 'done', downloadUrl: '//evil.com/x', title: 'T' })} onGenerate={() => {}} onDecline={() => {}} />)
