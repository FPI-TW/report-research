...
test('子標分「研報 · N／網路補充 · M」兩節；點研報卡開報告；ESC 關閉', () => {
  ...
  render(<SourcesDrawer open view={view({})} onClose={onClose} onOpenReport={onOpenReport} />)
...
test('closed 或 null 不渲染', () => {
  const { container } = render(<SourcesDrawer open={false} view={view({})} onClose={() => {}} onOpenReport={() => {}} />)
