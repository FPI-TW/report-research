...
export function KpiGrid({ progress }: { progress: Progress }) {
  ...
  return (
    <div className={styles.kpiGrid}>
      <KpiCard label="已導入報告" value={fmtInt(db.reports)} suffix="篇" sub={`${db.markets.length} 個市場`} />
      <KpiCard label="總片段 CHUNKS" value={fmtInt(db.chunks)} suffix="段" sub="向量片段總數" />
      <KpiCard
        label="標註進度"
        value={tagging ? tagging.pct.toFixed(1) : '—'}
        suffix="%"
