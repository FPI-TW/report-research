...
export function MarketDistribution({ markets, index = 0 }: { markets: MarketCount[]; index?: number }) {
  ...
  return (
    <div className={`${styles.card} ${styles.panel} ${styles.marginTop} tf-reveal`} style={{ ['--tf-i' as string]: index }}>
            ...
            return (
              <div key={code} className={styles.mktRow}>
                ...
                <div className={styles.mktBar}>
                  <div className={styles.mktFill} style={{ width: `${((m.count / max) * 100).toFixed(1)}%`, background: color, ['--tf-i' as string]: i }} />
                </div>
