...
test('6 列名稱 + 狀態徽章對應布林', () => {
  render(
    <PipelineStatus
      pipelines={{ web: true, ingest: false, tag: true, summaries: false, takeaways: true, signals: true }}
    />,
  )
  for (const n of ['Web 服務', '報告導入', '語意標註', '摘要生成', '重點摘錄', '觀點訊號']) {
    expect(screen.getByText(n)).toBeInTheDocument()
  }
  expect(screen.getAllByText('執行中')).toHaveLength(4)   // web + tag + takeaways + signals
  expect(screen.getAllByText('已停止')).toHaveLength(2)   // ingest + summaries
})

test('列順序固定為 web/ingest/tag/summaries/takeaways/signals', () => {
  const { container } = render(
    <PipelineStatus
      pipelines={{ web: true, ingest: true, tag: true, summaries: true, takeaways: true, signals: true }}
    />,
  )
  const names = [...container.querySelectorAll('[class*="pipeName"]')].map(el => el.textContent)
  expect(names).toEqual(['Web 服務', '報告導入', '語意標註', '摘要生成', '重點摘錄', '觀點訊號'])
})

// 滾動部署：後端還沒上線時 takeaways／signals 是 undefined。兩列仍要在（顯示已停止），
// 不可因為缺鍵而消失或渲染成空白徽章——否則「看不到那一列」與「批次沒在跑」兩種
// 情況在畫面上無法區分，正是這兩列要修掉的那個歧義。
test('後端未回 takeaways／signals 時兩列仍在並顯示已停止', () => {
  ...
  expect(screen.getAllByText('已停止')).toHaveLength(5)   // ingest+tag+summaries+takeaways+signals
})

test('PIPELINE_ROWS 是頁首分母的單一來源', () => {
  expect(PIPELINE_ROWS.map(r => r.key)).toEqual([
    'web', 'ingest', 'tag', 'summaries', 'takeaways', 'signals',
  ])
})
