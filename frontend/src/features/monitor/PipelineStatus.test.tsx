  ...
  render(
    <PipelineStatus
      pipelines={{
        ...
        summaries: false, takeaways: true, signals: true,
      }}
    />,
  )
  for (const n of ['Web 服務', '報告導入', '增量匯入', '語意標註', '摘要生成', '重點摘錄', '觀點訊號']) {
    expect(screen.getByText(n)).toBeInTheDocument()
  }
  expect(screen.getAllByText('執行中')).toHaveLength(5)   // web + sync_import + tag + takeaways + signals
  expect(screen.getAllByText('已停止')).toHaveLength(2)   // ingest + summaries
})

test('列順序固定為 web/ingest/sync_import/tag/summaries/takeaways/signals', () => {
  const { container } = render(
    <PipelineStatus
      pipelines={{
        web: true, ingest: true, sync_import: true, tag: true,
        summaries: true, takeaways: true, signals: true,
      }}
    />,
  )
  const names = [...container.querySelectorAll('[class*="pipeName"]')].map(el => el.textContent)
  expect(names).toEqual([
    'Web 服務', '報告導入', '增量匯入', '語意標註', '摘要生成', '重點摘錄', '觀點訊號',
  ])
})
...
test('後端未回 takeaways／signals／sync_import 時三列仍在並顯示已停止', () => {
  ...
  // ingest+sync_import+tag+summaries+takeaways+signals
  expect(screen.getAllByText('已停止')).toHaveLength(6)
})
