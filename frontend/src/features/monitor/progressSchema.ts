 ...
 *
 * `signals` 用 optional 的理由同 progressSchema 下方那三塊：滾動部署期間前端可能
 * 先上線，缺鍵就整張監控頁 parse 失敗變空白，代價遠大於少一列。
 * 未涵蓋 `generate_titles.py` 與 `extract_takeaways.py`——它們同樣沒有表徵，
 * 要補是同一個模式（後端加一格 + 這裡加一個 optional + ROWS 加一列）。
 */
