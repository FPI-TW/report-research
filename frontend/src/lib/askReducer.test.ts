...
test('正常 RAG：sources→retrieved→reading→generating→token→done', () => {
  ...
  s = askReducer(s, ev({ event: 'done', data: { cited: ['r1'], qa_id: 'qa1', conversation_id: 'c1', thinking_ms: 4200 } }))
  const t = s.turns[0]
...
test('離題：notice→done 保持 notice、qaId 為 null', () => {
  let s = submit()
...
test('feedback / reset / load', () => {
  ...
  const turn = turnFromHistory({ id: 'qa9', question: 'H', answer: 'A', created_at: '2026-06-20T00:00:00Z', feedback: 'dislike', sources: [], ext_sources: [], is_offtopic: false, thinking_ms: 1500, stages: [], followups: [], root_qa_id: null, version_count: 1, stopped: false })
...
test('turnFromHistory 離題轉 notice、qaId null', () => {
  const t = turnFromHistory({ id: 'qaX', question: 'H', answer: '無法回答此問題', created_at: null, feedback: null, sources: [], ext_sources: [], is_offtopic: true, thinking_ms: null, stages: [], followups: [], root_qa_id: null, version_count: 1, stopped: false })
  expect(t.phase).toBe('notice')
  expect(t.noticeText).toBe('無法回答此問題')
  expect(t.qaId).toBeNull()
...
test('turnFromHistory 帶出 notice_kind（重播不該把時效婉拒當成離題）', () => {
  const t = turnFromHistory({ id: 'qaY', question: 'H', answer: '需要即時行情', created_at: null, feedback: null, sources: [], ext_sources: [], is_offtopic: true, notice_kind: 'time_sensitive', thinking_ms: null, stages: [], followups: [], root_qa_id: null, version_count: 1, stopped: false })
  expect(t.phase).toBe('notice')
