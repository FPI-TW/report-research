...
/** 按壓 spring：快、帶一點回彈，讓 whileTap/whileHover 有「Q 彈」手感（全站 Pressable 用）。 */
export const springPress: Transition = { type: 'spring', stiffness: 520, damping: 24, mass: 0.7 }
/** 懸停/浮起 spring：較柔的落定，用於卡片 hover 上抬。 */
export const springHover: Transition = { type: 'spring', stiffness: 340, damping: 22 }
/** 分段控制滑動 thumb 的 layout spring（layoutId 滑移用）。 */
export const springThumb: Transition = { type: 'spring', stiffness: 460, damping: 34 }
...
/** 進場淡入＋上浮（原 .tf-reveal / @keyframes tf-up 的 Motion 版）；上浮距離加大讓波浪進場更有感。 */
export const revealVariants: Variants = {
  hidden: { opacity: 0, y: 14 },
  visible: { opacity: 1, y: 0 },
}

/** 進場 transition：reduced 走即時；否則 spring 落定（帶一點回彈），delay 依 index 封頂 stagger 形成波浪。 */
...
/** 進場變體：up＝淡入上浮（預設）、fade＝純淡入（密集文字/表格）、scale＝淡入縮放（放大回彈更明顯）。 */
...
export function revealVariantsFor(variant: RevealVariant): Variants {
  ...
  if (variant === 'scale') return { hidden: { opacity: 0, scale: 0.94 }, visible: { opacity: 1, scale: 1 } }
 ...
 */
export const tapPress = { whileTap: { scale: 0.94 }, transition: springPress } as const
/** 較含蓄的按壓（寬版/連結型控制）。 */
export const tapPressSoft = { whileTap: { scale: 0.96 }, transition: springPress } as const
/** 卡片/浮起型：hover 上抬、press 微縮（spring 落定）。 */
export const hoverLift = { whileHover: { y: -4 }, whileTap: { scale: 0.97 }, transition: springHover } as const
